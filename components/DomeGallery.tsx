...
export default function DomeGallery({
  ...
  // Initialize anime.js optimization
  useEffect(() => {
  ...
  // Handle item click with anime feedback
  const handleImageClick = useCallback((element: HTMLElement) => {
    // Add click pulse animation with anime.js
    anime({
      targets: element,
      scale: [1, 1.1, 1],
      duration: 300,
    ...
    // Trigger the original open function
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    // Enhanced animation with anime.js for smoother opening
    anime({
      targets: overlay,
      opacity: [0, 1],
      duration: enlargeTransitionMs,
    ...
    const checkAndShowLabels = () => {
      if (rootRef.current?.getAttribute('data-enlarging') === 'true') {
        // Anime.js timeline for synchronized label animations
        anime.timeline({
          duration: 600,
  ...
  const cssStyles = `
    ...
    @keyframes imageGlow {
      0%, 100% {
        box-shadow: 0 0 8px rgba(236, 72, 153, 0.3), inset 0 0 8px rgba(236, 72, 153, 0.1);
      }
      50% {
        box-shadow: 0 0 16px rgba(236, 72, 153, 0.5), inset 0 0 12px rgba(236, 72, 153, 0.2);
      }
    }
    ...
    .sphere-item {
      ...
      animation: tileEntrance 600ms ease-out forwards;
      animation-delay: calc(var(--offset-x) * 20ms + var(--offset-y) * 20ms);
    ...
    .item__image {
      ...
      box-shadow: 0 8px 20px rgba(0, 0, 0, 0.3), inset 0 0 8px rgba(236, 72, 153, 0.15);
      position: relative;
    }

    .item__image:hover {
      box-shadow: 0 12px 30px rgba(0, 0, 0, 0.4), inset 0 0 12px rgba(236, 72, 153, 0.25), 0 0 20px rgba(236, 72, 153, 0.4);
    ...
    .romantic-label {
      ...
      z-index: 40;
      pointer-events: none;
      text-shadow: 0 0 20px rgba(255, 105, 180, 0.5), 0 0 40px rgba(236, 72, 153, 0.3);
      font-weight: 300;
      letter-spacing: 2px;
    }
                ...
                >
                  <div
                    ...
                    onClick={e => {
                      ...
                      if (openingRef.current) return;
                      handleImageClick(e.currentTarget as HTMLElement);
                    }}
                    onPointerUp={e => {
